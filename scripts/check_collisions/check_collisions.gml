...
with(_objectid) {
	...
	if(dir = DIR_UP_RIGHT) {
		var tx = (x + _right_offset) >> TILE_SHIFT;
		var tx1 = (x - _left_offset - 4) >> TILE_SHIFT;
		var ty = (y - _top_offset - 4) >> TILE_SHIFT
		var ty1 = (y + _bottom_offset) >> TILE_SHIFT;

		var tile1 = tilemap_get(collision_map, tx, ty) & tile_index_mask;
		var tile2 = tilemap_get(collision_map, tx1, ty1) & tile_index_mask;
